...
const FileUpload = () => {
  ...
  return (
    <Container>
      ...
      {/* 파일 업로드 */}
      <UploadSection>
        <FileInput type="file" onChange={handleFileSelect} disabled={isUploading} />
        <ButtonGroup>
          <UploadButton onClick={() => handleUpload('')} disabled={!selectedFile || isUploading}>
            일반 업로드
          </UploadButton>
          <ProfileButton onClick={() => handleUpload('profile/')} disabled={!selectedFile || isUploading}>
            프로필 업로드
          </ProfileButton>
          <ProfileButton onClick={() => handleUpload('patient/')} disabled={!selectedFile || isUploading}>
            환자 프로필 업로드
          </ProfileButton>
        ...
        {files.length > 0 ? (
          files.map((file) => (
            <FileItem key={file.id}>
              <FileName>{file.originName}</FileName>
              <ButtonWrapper>
                {isImage(file.originName) && (
                  <DownloadButton onClick={() => handlePreview(file.changeName)}>미리보기</DownloadButton>
                )}
                <DownloadButton onClick={() => handleDownload(file.id, file.originalName)}>다운로드</DownloadButton>
              </ButtonWrapper>
            </FileItem>
          ))
        ) : (
      ...
      {previewUrl && (
        ...
        >
          <div style={{ background: '#fff', padding: 20, borderRadius: 8, textAlign: 'center' }}>
            ...
            <div>
              <button onClick={() => setPreviewUrl(null)} style={{ marginTop: 10 }}>
                닫기
              </button>
            </div>
          </div>
