...
//                .getSingleResult();
    ...
    @Override
    public Page<Board> findByUserNo(Status status, Role role, String option, String keyword, Long userNo,
                                    Pageable pageable) {

        // 기본 SELECT 쿼리: 상태와 역할로 필터링
        String baseQuery = "SELECT b FROM Board b WHERE b.status = :status AND b.role = :role AND b.user.userNo = :userNo";
        // COUNT 쿼리: 전체 개수 구할 때 사용
        String countQuery = "SELECT COUNT(b) FROM Board b WHERE b.status = :status AND b.role = :role AND b.user.userNo = :userNo";
